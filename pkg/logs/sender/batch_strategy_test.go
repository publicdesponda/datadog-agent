...
func TestBatchStrategySendsPayloadWhenBufferIsFull(t *testing.T) {
	...
	s := NewBatchStrategy(input, output, flushChan, false, nil, LineSerializer, 100*time.Millisecond, 2, 2, "test", &identityContentType{}, metrics.NewNoopPipelineMonitor(""), &metrics.NoopUtilizationMonitor{})
	s.Start()
...
func TestBatchStrategySendsPayloadWhenBufferIsOutdated(t *testing.T) {
	...
	clk := clock.NewMock()
	s := newBatchStrategyWithClock(input, output, flushChan, false, nil, LineSerializer, timerInterval, 100, 100, "test", clk, &identityContentType{}, metrics.NewNoopPipelineMonitor(""), &metrics.NoopUtilizationMonitor{})
	s.Start()
...
func TestBatchStrategySendsPayloadWhenClosingInput(t *testing.T) {
	...
	clk := clock.NewMock()
	s := newBatchStrategyWithClock(input, output, flushChan, false, nil, LineSerializer, 100*time.Millisecond, 2, 2, "test", clk, &identityContentType{}, metrics.NewNoopPipelineMonitor(""), &metrics.NoopUtilizationMonitor{})
	s.Start()
...
func TestBatchStrategySynchronousFlush(t *testing.T) {
	...
	strategy := NewBatchStrategy(input, output, flushChan, false, nil, LineSerializer, time.Hour, 100, 100, "test", &identityContentType{}, metrics.NewNoopPipelineMonitor(""), &metrics.NoopUtilizationMonitor{})
	strategy.Start()
